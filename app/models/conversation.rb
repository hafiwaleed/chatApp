class Conversation < ApplicationRecord
	...
	# class method between between method to find conversation between 2 users 
	# regardless who is sender or receiver
	...
	def unread_message_count(current_user)
		self.messages.where("user_id != ? AND read = ?",
			current_user.id, false).count
	end
end